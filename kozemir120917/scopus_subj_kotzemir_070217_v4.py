...
#working    entertext = u"""pubyear = 2003 and (DOCTYPE (ar) or DOCTYPE (cp) or DOCTYPE (re)) and affilcountry("Australia") and subjmain(""" + dicts_from_file[i].rstrip() + """)"""
    entertext = u"""DOCTYPE(ar or cp or re) and affilcountry(""" + countryname + """)  and pubyear > 2009 and pubyear < 2017 and (SUBJMAIN(2203) or SUBJMAIN(3313))"""
#    entertext = u"""af-ID(""" + dicts_from_file[i].rstrip() + """) and (DOCTYPE(ar or cp or re) and pubyear > 2009 and pubyear < 2017)"""
#af-ID(60008088) and (DOCTYPE(ar or cp or re) and pubyear > 2009 and pubyear < 2017)
...
#    driver.execute_script("arguments[0].setAttribute('value', arguments[1])", inputField, entertext)
    ...
    with open("output/" + "JSON_"+ filename + "_" + str(i) + ".txt", "w") as myfile:
    #print str(afil)
